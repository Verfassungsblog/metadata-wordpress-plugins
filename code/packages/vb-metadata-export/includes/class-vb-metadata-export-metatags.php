...
if ( ! class_exists( 'VB_Metadata_Export_Metatags' ) ) {
	...
	class VB_Metadata_Export_Metatags {
		...
		protected function get_ddc_subjects( $post ) {
			...
			array_walk($trimmed_ddc, function(&$value, $key) { $value = 'ddc:' . $value; } );
			return $trimmed_ddc;
		}
		...
		protected function get_gnd_subjects ( $post ) {
			...
			$gnd_terms = array_map( function ( $tag ) { return $tag->name; }, $gnd_terms);
			return $gnd_terms;
		}
		...
		protected function get_tag_subjects ( $post ) {
			...
			$tag_subjects = array_map( function ( $tag ) { return $tag->name; }, $tag_subjects);
			return $tag_subjects;
		}
		...
		public function render_highwire_keywords( $post ) {
			...
			if ( ! empty($subjects) ) {
				return $this->render_metatag("citation_keywords", implode("; ", $subjects));
			}
			return false;
		}
		...
		public function render_authors( $post, $meta_name ) {
			...
			$coauthors = array_filter( array_map(
				function ( $coauthor ) { return $this->common->get_coauthor_name( $coauthor ); },
				$coauthors
			) );
